...
import blogFakerFactory from "./blogFakerFactory";
...
const BlogFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = blogFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("blog").create(elem));
...
export default connect(null, mapDispatch)(BlogFakerDialogComponent);
